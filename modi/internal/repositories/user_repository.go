// Package repositories provides data access layer for the Modi application.
...
func (r *userRepository) Create(ctx context.Context, user *models.User) error {
	...
	if err != nil {
		// Check for constraint violations using error string (pgx/v5 error handling)
		errStr := err.Error()
		if strings.Contains(errStr, "duplicate key") ||
			strings.Contains(errStr, "unique constraint") ||
			strings.Contains(errStr, "violates unique constraint") ||
			strings.Contains(errStr, "23505") { // PostgreSQL unique_violation error code
...
// Delete deletes a user from the database (hard delete for now, can be changed to soft delete for GDPR).
